...
<head>
    ...
    <script>
        function fillForm(pid, adate, pname, agender, adoctor,
                          atime, atype, aproblem, aphone) {
            document.getElementById('pid').value = pid;
            document.getElementById('adate').value = adate;
            document.getElementById('pname').value = pname;
            document.getElementById('agender').value = agender;
            document.getElementById('adoctor').value = adoctor;
            document.getElementById('atime').value = atime;
            document.getElementById('atype').value = atype;
            document.getElementById('aproblem').value = aproblem;
...
<div class="container">
    <div class="form-box">
        <div class="form-title"><i class="bi bi-calendar-plus"></i> Book Appointment</div>
        <form action="${pageContext.request.contextPath}/appointment" method="post">
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="text" id="adoctor" name="adoctor" placeholder="Doctor Name"/>
                </div>
                <div class="form-group">
                    <label>Time</label>
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="atype" name="atype" placeholder="Cardiologist..."/>
                </div>
                <div class="form-group">
                    <label>Problem</label>
                    <input type="text" id="aproblem" name="aproblem" placeholder="Problem"/>
                </div>
            </div>
            <div class="form-row">
                <div class="form-group">
                    <label>Phone</label>
                    <input type="text" id="aphone" name="aphone" placeholder="Phone number"/>
                </div>
            </div>
            <div class="btn-row">
    ...
    <div class="table-box">
        <div class="table-title"><i class="bi bi-table"></i> All Appointments</div>
        <table>
            <tr>
                <th>ID</th><th>Date</th><th>Name</th><th>Gender</th>
                <th>Doctor</th><th>Time</th><th>Type</th><th>Problem</th><th>Phone</th>
            </tr>
            <% if(appointmentList != null && !appointmentList.isEmpty()) {
                for(Appointment a : appointmentList) { %>
            <tr onclick="fillForm('<%= a.getPid() %>','<%= a.getAdate() %>',
                '<%= a.getPname() %>','<%= a.getAgender() %>',
                '<%= a.getAdoctor() %>','<%= a.getAtime() %>',
                '<%= a.getAtype() %>','<%= a.getAproblem() %>',
                '<%= a.getAphone() %>')">
                <td><%= a.getPid() %></td>
                <td><%= a.getAdate() %></td>
