...
<html>
<head>
    <title>My Bills - HealthSync</title>
    ...
    <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .total-cell { background:#D1D2F9; font-weight:800; color:#0077b6; }
...
<div class="container">
    ...
    <div class="table-box">
        <div class="table-title"><i class="bi bi-receipt"></i> My Bills</div>
        <table>
            <tr>
                <th>ID</th><th>Name</th><th>Doctor Fees</th>
                <th>Room Fees</th><th>Extra Fees</th><th>Total Fees</th>
            </tr>
            <%
                try {
                    Connection con = DBConnection.getConnection();

                    // First get pname from patientdetails using username
                    PreparedStatement ptsName = con.prepareStatement(
                        "SELECT pname FROM patientdetails " +
                        "WHERE pname IN (SELECT pname FROM patientdetails) " +
                        "AND LOWER(pname) LIKE LOWER(?)");
                    ptsName.setString(1, "%" + username + "%");
                    ResultSet rsName = ptsName.executeQuery();

                    String pname = username; // fallback
                    if(rsName.next()) {
                        pname = rsName.getString("pname");
                    }

                    // Now get bills using pname
                    PreparedStatement pts = con.prepareStatement(
                        "SELECT * FROM medical_bills WHERE name=?");
                    pts.setString(1, pname);
            ...
            <tr>
                ...
                <td><%= rs.getString("doc_fees") %></td>
                <td><%= rs.getString("room_fees") %></td>
                <td><%= rs.getString("extra_fees") %></td>
                <td class="total-cell">₹ <%= rs.getString("total_fees") %></td>
            </tr>
            <%
                    }
                    if(!found) {
